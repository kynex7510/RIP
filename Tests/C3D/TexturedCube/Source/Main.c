...
static bool loadTexture(C3D_Tex* tex, const char* path) {
    ...
    // Do conversion in place, so we can reuse the buffer for all faces.
    if (!ripConvertInPlaceAndLoadC3DTexImage(tex, data, 0, 0)) {
        stbi_image_free(data);
        return false;
    }

    for (size_t face = 1; face < 6; ++face)
        C3D_TexLoadImage(tex, data, face, 0);
...
static void sceneInit(void)
{
    ...
    if (!loadTexture(&kitten_tex, "romfs:/Kitten.png"))
